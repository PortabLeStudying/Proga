...
| `push`, `pop`    | `void`, `char`     | Операции со стеком                              |
| `isBalanced`     | `int`              | Проверяет баланс скобок                         |
| `evaluatePostfix`| `int`              | Вычисляет постфиксное выражение                |
| `generatePascalTriangle` | `void`   | Генерирует треугольник Паскаля                 |
| `gcd`            | `int`              | Находит наибольший общий делитель               |
| `processFraction`| `void`             | Выполняет операцию над дробями                 |
